...
Voice = AI 写给未来自己的一条"内心声音"，五种形态：
  - question  闸门质问：到某个关卡（before_commit 等）必须自问的问题
  - note      便签：碰到特定关键词/分类就冒出来的提醒（事件前瞻记忆）
  - alarm     闹钟：到点必响（时间前瞻记忆，如"23:00 睡觉"）
  - task      任务提醒：以任务为锚的事件型闹钟——"完成 X 时提醒我做 Y"
              （如"睡觉时给手机充电"；延迟 N 分钟类提醒主流工具早已
              做好，这里补的是"完成某事时顺带要做某事"的前瞻记忆）
  - promise   承诺：把"马上做 X"的口头承诺落成账。到期未兑现由守护
              进程重叩催办（响一声就完的是闹钟；承诺不兑现不停止），
              直到带证据 fulfill 或留痕 release——防"答应即终止"
...
class Voice:
    id: int = 0
    kind: str = "question"            # question | note | alarm | task
    ...
    bind_task: str = ""               # task 专用：锚定的任务描述（完成它即触发）
...
class Ping:
    ...
    text: str = ""
    priority: int = 3
    source: str = "alarm"   # alarm（守护进程）| gate（闸门）| event（便签）| task（任务完成）
    fired_at: str = ""
    answered_at: str = ""
    answer: str = ""

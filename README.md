...
| [inner-voice-mcp](inner-voice-mcp/) | 内心声音：AI 给自己设的闸门质问、闹钟、便签、反思；独立守护进程，**会话全关闹钟仍在走** | 13 |
...
- **AI 会提醒自己**——"睡前给手机充电"式的前瞻记忆：闸门质问（before_commit 前问自己"测试全绿了吗"）+ 守护进程闹钟
...
接入 MCP 客户端（Claude Desktop / Cursor / Trae 等），参考 [`mcp-servers.example.json`](mcp-servers.example.json)：
...
三个文件夹保持并排放置（后两个按相对路径桥接记忆库）。详细的导入步骤、环境变量、验证清单见 [IMPORT.md](IMPORT.md)。
...
148 项断言 + 端到端场景 17 步，CI 覆盖 Ubuntu / Windows × Python 3.10–3.12。
